...
## PHASE 5 — full Fathom feature parity (user request: "clone every feature")
Keyless-first as always (demo fallbacks; real path when keys exist). Everything below ships:
**A. Library & organization:** folders (create/rename/delete, move calls, folder pages in sidebar), tabs My calls / Shared with me / Team, filters (meeting type, participant, company, date range, has action items), sort, rename + delete call, star calls, bulk move.
**B. Capture:** in-browser recorder `/record` (mic, optional screen/tab via getDisplayMedia, MediaRecorder, live timer + level meter) → same upload→process pipeline; "Send Fanthom to a live meeting" (paste Zoom/Meet/Teams URL → bot state machine: joining → waiting room → recording → processing; honest "bot is simulated" label; in demo mode it ends by creating a meeting from a seed template); `/calendar` week view of upcoming meetings with per-meeting Record toggle + auto-record rules (all / external only / internal only / none); video media support in the player (media_kind video renders <video> instead of the stage).
**C. Insights:** per-call coaching metrics panel (talk ratio per speaker, longest monologue, questions asked, filler words, interruptions, words-per-minute, patience) computed from segments; `/insights` team dashboard across calls (trends, per-person averages, meeting load); keyword **trackers** (define keywords/phrases → hits across calls with timestamps, tracker page); `/deals` companies view grouping external calls by email domain (company timeline, stakeholders, latest summary, next steps, BANT/MEDDPICC fields).
**D. Collaboration & export:** timestamped comments with @mentions on the call page (comment markers on timeline), emoji reactions on transcript lines; downloads: transcript TXT/SRT/VTT/Markdown, summary Markdown, recording file; clip trim editor (drag start/end handles, preview) when creating/editing highlights; integrations: REAL outgoing webhooks (Zapier-style, user-configured URL, POST meeting.ready payload + "Send test"), "Send to Slack" via Slack incoming-webhook URL (real), CRM sync preview (HubSpot/Salesforce field mapping of summary fields, "Sync" stub with log), email recap preview.
**E. Account & team:** Supabase Auth (magic link) when keys exist, otherwise demo user "Priya Raman" with a "Demo workspace" badge; `/team` members list (seed ~14 Northwind people), invite (stub), roles; onboarding flow `/welcome` (connect calendar stub → pick default template → record first meeting); notification center (bell: meeting ready, mentions, shared with you).
**F. Marketing:** `/pricing` (Free / Premium / Team / Business tiers in our own words), `/features`, `/integrations`, FAQ, footer; landing links to them.
